...
var certUsage = `
Usage:
    watchersid cert <subcommand> [flags]
...
Run ` + "`watchersid cert <subcommand> --help`" + ` for subcommand specific usage instructions.

`

var certInstallUsage = `
Usage:
    watchersid cert install [flags]
	
Installs a certificate to the system trust store, and (optionally) to the Firefox
and Java trust stores.

Options:
    --cert         Path to certificate. (Default: "~/.watchersid/watchersid_cert.pem")
...
Usage:
    watchersid cert uninstall [flags]
...
func NewCertInstallCommand(rootConfig *Config) *ffcli.Command {
	...
	fs := flag.NewFlagSet("watchersid cert install", flag.ExitOnError)

	fs.StringVar(&cmd.cert, "cert", "~/.watchersid/watchersid_cert.pem", "Path to certificate.")
...
func NewCertUninstallCommand(rootConfig *Config) *ffcli.Command {
	...
	fs := flag.NewFlagSet("watchersid cert uninstall", flag.ExitOnError)
